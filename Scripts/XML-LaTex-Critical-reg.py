...
for l in full_tree.iter(tei('l')):
	...
	if l.getchildren() != []:
		...
		for reg_pc in l.iter():
			if reg_pc.tag == tei('reg') or reg_pc.tag == tei('pc'):
				if reg_pc.findall('../..')[0].tag != tei('rdg'):
					line = line + reg_pc.text + ' '
			elif reg_pc.tag == tei('c'):
				line = line + " "
		...
		for app in l.iter(tei('app')):
			...
			for rdg in app.iter(tei('rdg')):
				...
				reading = rdg.findall('./'+tei('w')+'/'+tei('reg'))
				reading_string = ""
